DateInput = (function ($) {
	...
	DateInput.DEFAULT_OPTS = {
		...
		short_month_names: ["Янв", "Фев", "Мрт", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Нбр", "Дек"],
	...
	DateInput.prototype = {
		...
		selectDate: function (date) {
			...
			var selectDay = +/\d+/.exec(this.input.val());                       //выбранный день
			month = date.getMonth()

			var first_day = new Date(year, month, 1);

			var nextMonth = new Date(year, month + 1, 1);
			first_wday = first_day.getDay();

			first_wday === 0 ? first_wday = 7 : false;
			last_day = Math.ceil((nextMonth.getTime() - first_day.getTime() - oneHour) / oneDay);
			$(WhatMonth(year, month, selectDay));
